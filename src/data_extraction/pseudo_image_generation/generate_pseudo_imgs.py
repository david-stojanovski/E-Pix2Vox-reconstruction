...
def get_cropped_from_zoomed_img(in_image, in_cone):
    if type(in_image) is Image.Image:
...
def get_randomised_variables(cfg, view_augmentation_dict, view_name, dims):
    ...
    random_zoom_factor = uniform(
        view_augmentation_dict[view_name]["zoom_factor"][0],
        view_augmentation_dict[view_name]["zoom_factor"][1],
    )
    random_shift_x = int(
        uniform(
            view_augmentation_dict[view_name]["shift_x"][0],
            view_augmentation_dict[view_name]["shift_x"][1],
        )
    )
    random_shift_y = int(
        uniform(
            view_augmentation_dict[view_name]["shift_y"][0],
            view_augmentation_dict[view_name]["shift_y"][1],
        )
    )

    op = np.random.choice(
        [image_ops.brightening, image_ops.shadowing],
    ...
    loc_row = min(
        dims[0],
        max(
            cfg.PARAMS.LOC_MIN[0],
            np.random.normal(cfg.PARAMS.LOC_MIN[0], cfg.PARAMS.LOC_STD[0]),
        ),
    )
    loc_col = min(
        dims[1],
        max(
            cfg.PARAMS.LOC_MIN[1],
            np.random.normal(cfg.PARAMS.LOC_MIN[1], cfg.PARAMS.LOC_STD[1]),
        ),
    )
    brightness = min(
        1,
...
def get_img_data(in_img_path):
    ...
    """
    out_image = cv2.imread(in_img_path, 0) / 255.0
