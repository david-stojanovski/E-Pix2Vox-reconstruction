...
for _r_idx, r in enumerate(rooms):
    ...
    for ni in r["node_indices"]:
        ...
        if category in SHAPENET_OBJECTS and category not in [
            "02933112",
            "03691459",
            "02958343",
        ]:
        ...
        while (
            ...
            and category in SHAPENET_OBJECTS
            and category not in ["02933112", "03691459"]
        ):
    ...
    for ni in r["node_indices"]:
        ...
        for az in range(0, 360, 15):
            ...
            if _occluded <= OCCLUDED_LIMT:
                shutil.move(
                    clean_file_path,
                    os.path.join(output_folder, "render_%02d_clean.png" % img_counter),
                )
                shutil.move(
                    final_file_path,
                    os.path.join(output_folder, "render_%02d_final.png" % img_counter),
                )
                img_counter += 1
            else:
